...
function SimpleDialog(props) {
    const { onClose, selectedValue, open, playlists, id } = props;
    function addToPlaylist(playlistId){
        fetch(`http://localhost:4002/api/v2/endPoints/add/song/${playlistId}/${id}`,{
            method: "POST"
        })
        handleClose()
    }
  ...
  {/* ================= new playlist dialog ========================== */}
      <Dialog open={dialogOpen} onClose={handleDialogClose}>
        ...
        <DialogActions>
          ...
          <Button onClick={newPlaylist}>Subscribe</Button>
        </DialogActions>
      </Dialog>
      ...
      <List sx={{ pt: 0 }}>
        {playlists.map((playlist) => (
          ...
          <ListItem autoFocus key={playlist._id}>
            ...
            <ListItemText primary={playlist.name} onClick={()=>addToPlaylist(playlist._id)} />
          </ListItem>
        ))}
...
export default function PlaylistAddTo({playlists, id}) {
